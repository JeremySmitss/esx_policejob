...
RegisterServerEvent('gitta-noodknop')
AddEventHandler('gitta-noodknop', function(jobnaam,x,y,z)
    TriggerClientEvent('gitta-noodknop_client', -1, x,y,z, GetPlayerName(source), jobnaam)
end)

RegisterServerEvent('esx_policejob:revive') -- Helemaal Replacen
AddEventHandler('esx_policejob:revive', function(target)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'police' then
		TriggerClientEvent('esx_policejob:revive', target, source)
	else
		print(('esx_policejob: %s attempted to revive!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_policejob:buyItem', function(itemName, amount)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
	local sourceItem = xPlayer.getInventoryItem(itemName)
